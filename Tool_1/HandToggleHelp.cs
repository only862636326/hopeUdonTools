...
namespace HopeTools
{
    /// <summary>
    ///  copy form Thry.Sao 
    ///  https://github.com/Thryrallo/UdonPrefabs
    /// </summary>

    [UdonBehaviourSyncMode(BehaviourSyncMode.None)]
    public class HandToggleHelp : UdonSharpBehaviour
    {
        ...
        public Collider[] currentColliders = new Collider[10];
        public float[] currentCollidersEnterTime = new float[10];
        public Vector3[] currentCollidersEnterPosition = new Vector3[10];
        public bool[] currentCollidersBlockPress = new bool[10];
        public bool[] currentCollidersSupportsDragInteraction = new bool[10];
        ...
        private void Start()
        {
            ...
            if(!this.is_test && !_player.IsUserInVR())
            {
                this.gameObject.SetActive(false);
            }

            this.transform.GetChild(0).gameObject.SetActive(is_test);
        }

        public override void PostLateUpdate()
        {
            if (_isNotVR && !is_test) return;
            HandleDrag();
            if (isRightHand) SetToMostPreciseRightIndexFingerPosition(transform);
            else SetToMostPreciseLeftIndexFingerPosition(transform);
            transform.localScale = Vector3.one * (0.01f * this.GetLocalAvatarHeight());
        ...
        {
            //Debug.Log("enter gm obg : " + other.gameObject.name);
            if (_isNotVR && !is_test) return;
            if (other == null)
                return;
        ...
        UnityEngine.UI.Toggle clickToggle;
        ColorBlock clickedButtonPrevColorBlock;
        ColorBlock clickedBTogglePrevColorBlock;
        float clickedButtonTime = 0;
        float clickedToggleTime = 0;
        ...
        private void Press(Collider other)
        {
            ...
            if (!_blockInput && isMovingBack)
            {
                ...
                if (toggle != null)
                {
                    ...
                    if (clickToggle != null) clickToggle.colors = clickedBTogglePrevColorBlock;
                    SetClickedToggleColor(toggle);
        ...
        public void SetClickedToggleColor(UnityEngine.UI.Toggle toggle)
        {
            clickToggle = toggle;
            ColorBlock block = toggle.colors;
            clickedBTogglePrevColorBlock = block;
            block.normalColor = block.pressedColor;
            block.selectedColor = block.pressedColor;
            toggle.colors = block;
            clickedToggleTime = Time.time;
            SendCustomEventDelayedSeconds(nameof(ResetClickedTollgeColor), 0.3f);
        }
        ...
        public void ResetClickedTollgeColor()
        {
            if (clickToggle != null && Time.time - clickedToggleTime > 0.2f)
            {
                clickToggle.colors = clickedBTogglePrevColorBlock;
                clickToggle = null;
            }
        }
        ...
        private void Drag(Collider other, int index, Vector3 delta, Vector3 startPosition, float timeSinceClick)
        {
            ...
            if (scrollRect && (transform.position - startPosition).sqrMagnitude > scrollThresholdSquare * this.GetLocalAvatarHeight())
            {
                ...
                if (!isScrolling)
                {
                    ...
                    for (int i = 0; i < currentColliders.Length; i++)
                        currentCollidersBlockPress[i] = true;
                }
                scrollRect.verticalNormalizedPosition += -delta.y * scrollSpeed * scrollRect.verticalScrollbar.size / this.GetLocalAvatarHeight();
            }
