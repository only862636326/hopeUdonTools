...
namespace HopeTools
{
    ...
    {
        public const int MAX_TASK_NUM = 10;
        ...
        public Transform[] task_org_tf_list = new Transform[MAX_TASK_NUM];
        ...
        void Update()
        {
            if (task_org_tf_list == null || task_tar_tf_list == null || task_obj_tf_list == null)
                return;

            int count = Mathf.Min(task_org_tf_list.Length, task_tar_tf_list.Length, task_obj_tf_list.Length, MAX_TASK_NUM);

            for (int i = 0; i < count; i++)
            {
                if (!_taskActive[i]) continue;
                if (task_org_tf_list[i] == null || task_tar_tf_list[i] == null || task_obj_tf_list[i] == null) continue;
                ...
                Vector3 p0 = task_org_tf_list[i].position;
                Vector3 p2 = task_tar_tf_list[i].position;
                Vector3 p1 = (p0 + p2) * BEZIER_MIDPOINT + Vector3.up * bezierHeight;
                ...
                task_obj_tf_list[i].rotation = Quaternion.Slerp(task_org_tf_list[i].rotation, task_tar_tf_list[i].rotation, t);
            }
        }

        /// <summary>自动找空位启动一个移动任务，返回索引，没空位返回-1</summary>
        public int AssignTask(Transform obj, Transform org, Transform tar)
        {
            if (task_org_tf_list == null || task_tar_tf_list == null || task_obj_tf_list == null)
                return NO_SLOT;
        ...
        public void StartAllTasks()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (task_obj_tf_list[i] != null && task_org_tf_list[i] != null && task_tar_tf_list[i] != null)
                {
                    _taskProgress[i] = PROGRESS_MIN;
                    _taskActive[i] = true;
                }
